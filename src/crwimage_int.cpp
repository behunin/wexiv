...
const CrwMapping CrwMap::crwMapping_[] = {
    ...
    //         ------  ------  ---- ------- -----    ---------     ---------
    CrwMapping(0x0805, 0x300a, 0, 0, canonId, decode0x0805),
    CrwMapping(0x080a, 0x2807, 0, 0, canonId, decode0x080a),
    CrwMapping(0x080b, 0x3004, 0, 0x0007, canonId, decodeBasic),
    CrwMapping(0x0810, 0x2807, 0, 0x0009, canonId, decodeBasic),
    CrwMapping(0x0815, 0x2804, 0, 0x0006, canonId, decodeBasic),
    CrwMapping(0x1029, 0x300b, 0, 0x0002, canonId, decodeBasic),
    CrwMapping(0x102a, 0x300b, 0, 0x0004, canonId, decodeArray),
    CrwMapping(0x102d, 0x300b, 0, 0x0001, canonId, decodeArray),
    ...
    CrwMapping(0x10c1, 0x300b, 0, 0x00c1, canonId, decodeBasic),
    CrwMapping(0x1807, 0x3002, 0, 0x9206, exifId, decodeBasic),
    CrwMapping(0x180b, 0x3004, 0, 0x000c, canonId, decodeBasic),
...
void CiffComponent::doRead(const byte* pData, uint32_t size, uint32_t start, ByteOrder byteOrder) {
...
void CiffDirectory::readDirectory(const byte* pData, uint32_t size, ByteOrder byteOrder) {
  ...
  for (uint16_t i = 0; i < count; ++i) {
    ...
    switch (CiffComponent::typeId(tag)) {
      case directory:
        m = CiffComponent::UniquePtr(new CiffDirectory);
        break;
      default:
        m = CiffComponent::UniquePtr(new CiffEntry);
        break;
    }
    m->setDir(this->tag());
...
uint32_t CiffComponent::write(Blob& blob, ByteOrder byteOrder, uint32_t offset) {
...
void CrwMap::decode0x080a(const CiffComponent& ciffComponent, const CrwMapping* /*pCrwMapping*/, Image& image,
  ...
  uint32_t i = 0;
  for (; i < ciffComponent.size() && ciffComponent.pData()[i] != '\0'; ++i) {
    // empty
  }
  value1->read(ciffComponent.pData(), ++i, byteOrder);
  image.exifData().set("Exif.Image.Make", value1->toString());

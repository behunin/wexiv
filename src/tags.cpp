...
ExifKey::ExifKey(uint16_t tag, const std::string& groupName) : p_(new Impl) {
  IfdId ifdId = groupId(groupName);
...
ExifKey::ExifKey(const TagInfo& ti) : p_(new Impl) {
...
ExifKey::ExifKey(const std::string& key) : p_(new Impl) {
  p_->decomposeKey(key);
}

ExifKey::ExifKey(const ExifKey& rhs) : p_(new Impl(*rhs.p_)) {
}

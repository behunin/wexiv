...
constexpr TagDetails minoltaColorMode[] = {
    {0, N_("Natural Color")}, {1, N_("Black & White")}, {2, N_("Vivid Color")},  {3, N_("Solarization")},   {4, N_("AdobeRGB")},
    {5, N_("Sepia")},         {9, N_("Natural")},       {12, N_("Portrait")},    {13, N_("Natural sRGB")},  {14, N_("Natural+ sRGB")},
    {15, N_("Landscape")},    {16, N_("Evening")},      {17, N_("Night Scene")}, {18, N_("Night Portrait")}};

//! Lookup table to translate Minolta image quality values to readable labels
EXV_UNUSED constexpr TagDetails minoltaImageQuality[] = {{0, N_("Raw")},      {1, N_("Super Fine")}, {2, N_("Fine")},
...
constexpr TagInfo MinoltaMakerNote::tagInfo_[] = {
    {0x0000, "Version", N_("Makernote Version"), N_("String 'MLT0' (not null terminated)"), minoltaId, makerTags, undefined, -1,
     printValue},
    {0x0001, "CameraSettingsStdOld", N_("Camera Settings (Std Old)"),
     N_("Standard Camera settings (Old Camera models like D5, D7, S304, and S404)"), minoltaId, makerTags, undefined, -1, printValue},
    {0x0003, "CameraSettingsStdNew", N_("Camera Settings (Std New)"),
     N_("Standard Camera settings (New Camera Models like D7u, D7i, and D7hi)"), minoltaId, makerTags, undefined, -1, printValue},
    {0x0004, "CameraSettings7D", N_("Camera Settings (7D)"), N_("Camera Settings (for Dynax 7D model)"), minoltaId, makerTags,
     undefined, -1, printValue},
    {0x0018, "ImageStabilizationData", N_("Image Stabilization Data"), N_("Image stabilization data"), minoltaId, makerTags, undefined,
     -1, printValue},

    // TODO: Implement WB Info A100 tags decoding.
    {0x0020, "WBInfoA100", N_("WB Info A100"), N_("White balance information for the Sony DSLR-A100"), minoltaId, makerTags, undefined,
     -1, printValue},

    {0x0040, "CompressedImageSize", N_("Compressed Image Size"), N_("Compressed image size"), minoltaId, makerTags, unsignedLong, -1,
     printValue},
    {0x0081, "Thumbnail", N_("Thumbnail"), N_("Jpeg thumbnail 640x480 pixels"), minoltaId, makerTags, undefined, -1, printValue},
    {0x0088, "ThumbnailOffset", N_("Thumbnail Offset"), N_("Offset of the thumbnail"), minoltaId, makerTags, unsignedLong, -1,
     printValue},
    {0x0089, "ThumbnailLength", N_("Thumbnail Length"), N_("Size of the thumbnail"), minoltaId, makerTags, unsignedLong, -1,
     printValue},
    {0x0100, "SceneMode", N_("Scene Mode"), N_("Scene Mode"), minoltaId, makerTags, unsignedLong, -1, printMinoltaSonySceneMode},

    // TODO: for A100, use Sony table from printMinoltaSonyColorMode().
    {0x0101, "ColorMode", N_("Color Mode"), N_("Color mode"), minoltaId, makerTags, unsignedLong, -1, EXV_PRINT_TAG(minoltaColorMode)},
    ...
    {0x0104, "FlashExposureComp", N_("Flash Exposure Compensation"), N_("Flash exposure compensation in EV"), minoltaId, makerTags,
     signedRational, -1, print0x9204},
    {0x0105, "Teleconverter", N_("Teleconverter Model"), N_("Teleconverter Model"), minoltaId, makerTags, unsignedLong, -1,
     printMinoltaSonyTeleconverterModel},
    {0x0107, "ImageStabilization", N_("Image Stabilization"), N_("Image stabilization"), minoltaId, makerTags, unsignedLong, -1,
     EXV_PRINT_TAG(minoltaImageStabilization)},
    {0x0109, "RawAndJpgRecording", N_("RAW+JPG Recording"), N_("RAW and JPG files recording"), minoltaId, makerTags, unsignedLong, -1,
    ...
    {0x010b, "ColorTemperature", N_("Color Temperature"), N_("Color temperature"), minoltaId, makerTags, unsignedLong, -1, printValue},
    {0x010c, "LensID", N_("Lens ID"), N_("Lens identifier"), minoltaId, makerTags, unsignedLong, -1, printMinoltaSonyLensID},
    {0x0111, "ColorCompensationFilter", N_("Color Compensation Filter"),
     N_("Color Compensation Filter: negative is green, positive is magenta"), minoltaId, makerTags, unsignedLong, -1, printValue},
    {0x0112, "WhiteBalanceFineTune", N_("White Balance Fine Tune"), N_("White Balance Fine Tune Value"), minoltaId, makerTags,
     unsignedLong, -1, printValue},
    {0x0113, "ImageStabilizationA100", N_("Image Stabilization A100"), N_("Image Stabilization for the Sony DSLR-A100"), minoltaId,
     makerTags, unsignedLong, -1, printMinoltaSonyBoolValue},

    // TODO: implement CameraSettingsA100 tags decoding.
    {0x0114, "CameraSettings5D", N_("Camera Settings (5D)"), N_("Camera Settings (for Dynax 5D model)"), minoltaId, makerTags,
     undefined, -1, printValue},
    ...
    {0x0f00, "CameraSettingsZ1", N_("Camera Settings (Z1)"), N_("Camera Settings (for Z1, DImage X, and F100 models)"), minoltaId,
     makerTags, undefined, -1, printValue},
    // End of list marker
    {0xffff, "(UnknownMinoltaMakerNoteTag)", "(UnknownMinoltaMakerNoteTag)", N_("Unknown Minolta MakerNote tag"), minoltaId, makerTags,
...
constexpr TagDetails minoltaMeteringModeStd[] = {{0, N_("Multi-segment")}, {1, N_("Center weighted average")}, {2, N_("Spot")}};
...
EXV_UNUSED constexpr TagDetails minoltaAFPointsStd[] = {{0, N_("Center")},      {1, N_("Top")},          {2, N_("Top-right")},
                                                        {3, N_("Right")},       {4, N_("Bottom-right")}, {5, N_("Bottom")},
                                                        {6, N_("Bottom-left")}, {7, N_("Left")},         {8, N_("Top-left")}};
...
constexpr TagDetails minoltaSubjectProgramStd[] = {{0, N_("None")},           {1, N_("Portrait")}, {2, N_("Text")},
...
constexpr TagDetails minoltaISOSettingStd[] = {{0, "100"}, {1, "200"}, {2, "400"}, {3, "800"}, {4, N_("Auto")}, {5, "64"}};
...
constexpr TagDetails minoltaDECPositionStd[] = {{0, N_("Exposure")}, {1, N_("Contrast")}, {2, N_("Saturation")}, {3, N_("Filter")}};
...
std::ostream& MinoltaMakerNote::printMinoltaDateStd(std::ostream& os, const Value& value, const ExifData*) {
  ...
  os << value.toLong() / 65536 << ":" << std::right << std::setw(2) << std::setfill('0')
     << (value.toLong() - value.toLong() / 65536 * 65536) / 256 << ":" << std::right << std::setw(2) << std::setfill('0')
     << value.toLong() % 256;
  return os;
}
...
std::ostream& MinoltaMakerNote::printMinoltaTimeStd(std::ostream& os, const Value& value, const ExifData*) {
  ...
  os << std::right << std::setw(2) << std::setfill('0') << value.toLong() / 65536 << ":" << std::right << std::setw(2)
     << std::setfill('0') << (value.toLong() - value.toLong() / 65536 * 65536) / 256 << ":" << std::right << std::setw(2)
...
constexpr TagInfo MinoltaMakerNote::tagInfoCsStd_[] = {
    ...
    {0x0008, "ISO", N_("ISO"), N_("ISO Value"), minoltaCsNewId, makerTags, unsignedLong, 1, printMinoltaExposureSpeedStd},
    {0x0009, "ExposureTime", N_("Exposure Time"), N_("Exposure time"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printMinoltaExposureTimeStd},
    {0x000A, "FNumber", N_("FNumber"), N_("The F-Number"), minoltaCsNewId, makerTags, unsignedLong, 1, printMinoltaFNumberStd},
    {0x000B, "MacroMode", N_("Macro Mode"), N_("Macro mode"), minoltaCsNewId, makerTags, unsignedLong, 1, printMinoltaSonyBoolValue},
    ...
    {0x000D, "ExposureCompensation", N_("Exposure Compensation"), N_("Exposure compensation"), minoltaCsNewId, makerTags, unsignedLong,
     1, printMinoltaExposureCompensationStd},
    ...
    {0x0010, "IntervalLength", N_("Interval Length"), N_("Interval length"), minoltaCsNewId, makerTags, unsignedLong, 1, printValue},
    {0x0011, "IntervalNumber", N_("Interval Number"), N_("Interval number"), minoltaCsNewId, makerTags, unsignedLong, 1, printValue},
    {0x0012, "FocalLength", N_("Focal Length"), N_("Focal length"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printMinoltaFocalLengthStd},
    {0x0013, "FocusDistance", N_("Focus Distance"), N_("Focus distance"), minoltaCsNewId, makerTags, unsignedLong, 1, printValue},
    {0x0014, "FlashFired", N_("Flash Fired"), N_("Flash fired"), minoltaCsNewId, makerTags, unsignedLong, 1,
     EXV_PRINT_TAG(minoltaFlashFired)},
    {0x0015, "MinoltaDate", N_("Minolta Date"), N_("Minolta date"), minoltaCsNewId, makerTags, unsignedLong, 1, printMinoltaDateStd},
    {0x0016, "MinoltaTime", N_("Minolta Time"), N_("Minolta time"), minoltaCsNewId, makerTags, unsignedLong, 1, printMinoltaTimeStd},
    {0x0017, "MaxAperture", N_("Max Aperture"), N_("Max aperture"), minoltaCsNewId, makerTags, unsignedLong, 1, printValue},
    {0x001A, "FileNumberMemory", N_("File Number Memory"), N_("File number memory"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printMinoltaSonyBoolValue},
    {0x001B, "LastFileNumber", N_("Last Image Number"), N_("Last image number"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printValue},
    {0x001C, "ColorBalanceRed", N_("Color Balance Red"), N_("Color balance red"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printMinoltaWhiteBalanceStd},
    {0x001D, "ColorBalanceGreen", N_("Color Balance Green"), N_("Color balance green"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printMinoltaWhiteBalanceStd},
    {0x001E, "ColorBalanceBlue", N_("Color Balance Blue"), N_("Color balance blue"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printMinoltaWhiteBalanceStd},
    ...
    {0x0023, "FlashExposureComp", N_("Flash Exposure Compensation"), N_("Flash exposure compensation in EV"), minoltaCsNewId,
     makerTags, unsignedLong, 1, printMinoltaFlashExposureCompStd},
    ...
    {0x0029, "ColorFilter", N_("Color Filter"), N_("Color filter"), minoltaCsNewId, makerTags, unsignedLong, 1, printValue},
    {0x002A, "BWFilter", N_("Black and White Filter"), N_("Black and white filter"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printValue},
    {0x002B, "Internal Flash", N_("Internal Flash"), N_("Internal Flash"), minoltaCsNewId, makerTags, unsignedLong, 1,
     EXV_PRINT_TAG(minoltaFlashFired)},
    {0x002C, "Brightness", N_("Brightness"), N_("Brightness"), minoltaCsNewId, makerTags, unsignedLong, 1, printMinoltaBrightnessStd},
    {0x002D, "SpotFocusPointX", N_("Spot Focus Point X"), N_("Spot focus point X"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printValue},
    {0x002E, "SpotFocusPointY", N_("Spot Focus Point Y"), N_("Spot focus point Y"), minoltaCsNewId, makerTags, unsignedLong, 1,
     printValue},
    ...
    {0x0032, "DECPosition", N_("DEC Switch Position"), N_("DEC switch position"), minoltaCsNewId, makerTags, unsignedLong, 1,
     EXV_PRINT_TAG(minoltaDECPositionStd)},
    ...
    // End of list marker
    {0xffff, "(UnknownMinoltaCsStdTag)", "(UnknownMinoltaCsStdTag)", N_("Unknown Minolta Camera Settings tag"), minoltaCsNewId,
...
constexpr TagDetails minoltaWhiteBalance7D[] = {
    ...
    {5, N_("Fluorescent")}, {256, N_("Kelvin")}, {512, N_("Manual")}, {512, N_("Manual")} // To silence compiler warning
};
...
constexpr TagDetails minoltaColorSpace7D[] = {{0, N_("sRGB (Natural)")}, {1, N_("sRGB (Natural+)")}, {4, N_("Adobe RGB")}};

//! Lookup table to translate Minolta Dynax 7D camera settings rotation values to readable labels
constexpr TagDetails minoltaRotation7D[] = {{72, N_("Horizontal (normal)")}, {76, N_("Rotate 90 CW")}, {82, N_("Rotate 270 CW")}};
...
constexpr TagInfo MinoltaMakerNote::tagInfoCs7D_[] = {
    ...
    {0x001E, "ExposureCompensation", N_("Exposure Compensation"), N_("Exposure compensation"), minoltaCs7DId, makerTags, signedShort,
     1, printValue},
    ...
    {0x002D, "FreeMemoryCardImages", N_("Free Memory Card Images"), N_("Free memory card images"), minoltaCs7DId, makerTags,
     unsignedShort, 1, printValue},
    {0x003F, "ColorTemperature", N_("Color Temperature"), N_("Color temperature"), minoltaCs7DId, makerTags, signedShort, 1,
     printValue},
    {0x0040, "Hue", N_("Hue"), N_("Hue"), minoltaCs7DId, makerTags, unsignedShort, 1, printValue},
    {0x0046, "Rotation", N_("Rotation"), N_("Rotation"), minoltaCs7DId, makerTags, unsignedShort, 1, EXV_PRINT_TAG(minoltaRotation7D)},
    {0x0047, "FNumber", N_("FNumber"), N_("The F-Number"), minoltaCs7DId, makerTags, unsignedShort, 1, printValue},
    {0x0048, "ExposureTime", N_("Exposure Time"), N_("Exposure time"), minoltaCs7DId, makerTags, unsignedShort, 1, printValue},
    // 0x004A is a dupplicate than 0x002D.
    {0x004A, "FreeMemoryCardImages", N_("Free Memory Card Images"), N_("Free memory card images"), minoltaCs7DId, makerTags,
     unsignedShort, 1, printValue},
    {0x005E, "ImageNumber", N_("Image Number"), N_("Image number"), minoltaCs7DId, makerTags, unsignedShort, 1, printValue},
    ...
    {0x0062, "ImageNumber", N_("Image Number"), N_("Image number"), minoltaCs7DId, makerTags, unsignedShort, 1, printValue},
    {0x0071, "ImageStabilization", N_("Image Stabilization"), N_("Image stabilization"), minoltaCs7DId, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolValue},
    {0x0075, "ZoneMatchingOn", N_("Zone Matching On"), N_("Zone matching on"), minoltaCs7DId, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolValue},
    // End of list marker
    {0xffff, "(UnknownMinoltaCs7DTag)", "(UnknownMinoltaCs7DTag)", N_("Unknown Minolta Camera Settings 7D tag"), minoltaCs7DId,
...
constexpr TagDetails minoltaExposureMode5D[] = {
    {0, N_("Program")},     {1, N_("Aperture priority")}, {2, N_("Shutter priority")},         {3, N_("Manual")},
    {4, N_("Auto")},        {5, N_("Program Shift A")},   {6, N_("Program Shift S")},          {0x1013, N_("Portrait")},
    {0x1023, N_("Sports")}, {0x1033, N_("Sunset")},       {0x1043, N_("Night View/Portrait")}, {0x1053, N_("Landscape")},
    {0x1083, N_("Macro")}};
...
constexpr TagDetails minoltaRotation5D[] = {{72, N_("Horizontal (normal)")}, {76, N_("Rotate 90 CW")}, {82, N_("Rotate 270 CW")}};

//! Lookup table to translate Minolta Dynax 5D camera settings focus position values to readable labels
constexpr TagDetails minoltaFocusPosition5D[] = {{0, N_("Wide")},  {1, N_("Central")},    {2, N_("Up")},   {3, N_("Up right")},
                                                 {4, N_("Right")}, {5, N_("Down right")}, {6, N_("Down")}, {7, N_("Down left")},
                                                 {8, N_("Left")},  {9, N_("Up left")}};
...
constexpr TagDetails minoltaPictureFinish5D[] = {
    {0, N_("Natural")},     {1, N_("Natural+")},       {2, N_("Portrait")},   {3, N_("Wind Scene")}, {4, N_("Evening Scene")},
    {5, N_("Night Scene")}, {6, N_("Night Portrait")}, {7, N_("Monochrome")}, {8, N_("Adobe RGB")},  {9, N_("Adobe RGB (ICC)")}};
...
constexpr TagInfo MinoltaMakerNote::tagInfoCs5D_[] = {
    ...
    {0x0035, "ExposureTime", N_("Exposure Time"), N_("Exposure time"), minoltaCs5DId, makerTags, unsignedShort, -1, printValue},
    {0x0036, "FNumber", N_("FNumber"), N_("The F-Number"), minoltaCs5DId, makerTags, unsignedShort, -1, printValue},
    {0x0037, "FreeMemoryCardImages", N_("Free Memory Card Images"), N_("Free memory card images"), minoltaCs5DId, makerTags,
     unsignedShort, -1, printValue},
    {0x0038, "ExposureRevision", N_("Exposure Revision"), N_("Exposure revision"), minoltaCs5DId, makerTags, unsignedShort, -1,
     printValue},
    {0x0048, "FocusMode", N_("Focus Mode"), N_("Focus mode"), minoltaCs5DId, makerTags, unsignedShort, -1,
     EXV_PRINT_TAG(minoltaFocusModeStd)},
    {0x0049, "ColorTemperature", N_("Color Temperature"), N_("Color temperature"), minoltaCs5DId, makerTags, signedShort, -1,
     printValue},
    {0x0050, "Rotation", N_("Rotation"), N_("Rotation"), minoltaCs5DId, makerTags, unsignedShort, -1,
     EXV_PRINT_TAG(minoltaRotation5D)},
    {0x0053, "ExposureCompensation", N_("Exposure Compensation"), N_("Exposure compensation"), minoltaCs5DId, makerTags, unsignedShort,
     -1, printMinoltaExposureCompensation5D},
    {0x0054, "FreeMemoryCardImages", N_("Free Memory Card Images"), N_("Free memory card images"), minoltaCs5DId, makerTags,
     unsignedShort, -1, printValue},
    {0x0065, "Rotation2", N_("Rotation2"), N_("Rotation2"), minoltaCs5DId, makerTags, unsignedShort, -1, printMinoltaSonyRotation},
    {0x006E, "Color Temperature", N_("Color Temperature"), N_("Color Temperature"), minoltaCs5DId, makerTags, signedShort, -1,
     printValue},
    ...
    {0x0091, "ExposureManualBias", N_("Exposure Manual Bias"), N_("Exposure manual bias"), minoltaCs5DId, makerTags, unsignedShort, -1,
     printMinoltaExposureManualBias5D},
    {0x009E, "AFMode", N_("AF Mode"), N_("AF mode"), minoltaCs5DId, makerTags, unsignedShort, -1, EXV_PRINT_TAG(minoltaAFMode5D)},
    {0x00AE, "ImageNumber", N_("Image Number"), N_("Image number"), minoltaCs5DId, makerTags, unsignedShort, -1, printValue},
    {0x00B0, "NoiseReduction", N_("Noise Reduction"), N_("Noise reduction"), minoltaCs5DId, makerTags, unsignedShort, -1,
     printMinoltaSonyBoolValue},
    {0x00BD, "ImageStabilization", N_("Image Stabilization"), N_("Image stabilization"), minoltaCs5DId, makerTags, unsignedShort, -1,
     printMinoltaSonyBoolValue},
    ...
    // End of list marker
    {0xFFFF, "(UnknownMinoltaCs5DTag)", "(UnknownMinoltaCs5DTag)", N_("Unknown Minolta Camera Settings 5D tag"), minoltaCs5DId,
     makerTags, invalidTypeId, -1, printValue},
};
...
constexpr TagDetails sonyFlashModeA100[] = {{0, N_("Auto")}, {2, N_("Rear flash sync")}, {3, N_("Wireless")}, {4, N_("Fill flash")}};

//! Lookup table to translate Sony A100 camera settings metering mode values to readable labels
constexpr TagDetails sonyMeteringModeA100[] = {{0, N_("Multi-segment")}, {1, N_("Center weighted average")}, {2, N_("Spot")}};
...
constexpr TagDetails sonyAELButtonA100[] = {{0, N_("Hold")}, {1, N_("Toggle")}, {2, N_("Spot Hold")}, {3, N_("Spot Toggle")}};
...
constexpr TagInfo MinoltaMakerNote::tagInfoCsA100_[] = {
    {0x0000, "ExposureMode", N_("Exposure Mode"), N_("Exposure mode"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(minoltaExposureMode5D)},
    {0x0001, "ExposureCompensationSetting", N_("Exposure Compensation Setting"), N_("Exposure compensation setting"), sony1MltCsA100Id,
     makerTags, unsignedShort, 1, printValue},
    {0x0005, "HighSpeedSync", N_("High Speed Sync"), N_("High speed sync"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolValue},
    {0x0006, "ManualExposureTime", N_("Manual Exposure Time"), N_("Manual exposure time"), sony1MltCsA100Id, makerTags, unsignedShort,
     1, printValue},
    {0x0007, "ManualFNumber", N_("Manual FNumber"), N_("Manual FNumber"), sony1MltCsA100Id, makerTags, unsignedShort, 1, printValue},
    {0x0008, "ExposureTime", N_("Exposure Time"), N_("Exposure time"), sony1MltCsA100Id, makerTags, unsignedShort, 1, printValue},
    ...
    {0x000D, "LocalAFAreaPoint", N_("Local AF Area Point"), N_("Local AF Area Point"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
    ...
    {0x0013, "ISOSetting", N_("ISO Setting"), N_("ISO setting"), sony1MltCsA100Id, makerTags, unsignedShort, 1, printValue},
    {0x0014, "ZoneMatchingMode", N_("Zone Matching Mode"), N_("Zone Matching Mode"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyZoneMatchingModeA100)},
    {0x0015, "DynamicRangeOptimizerMode", N_("Dynamic Range Optimizer Mode"), N_("Dynamic range optimizer mode"), sony1MltCsA100Id,
    ...
    {0x001A, "Saturation", N_("Saturation"), N_("Saturation"), sony1MltCsA100Id, makerTags, unsignedShort, 1, printValue},
    ...
    {0x001F, "SelfTimerTime", N_("Self Timer Time"), N_("Self timer time"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonySelfTimerTimeA100)},
    {0x0020, "ContinuousBracketing", N_("Continuous Bracketing"), N_("Continuous bracketing"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonyContinuousBracketingA100)},
    {0x0021, "SingleFrameBracketing", N_("Single Frame Bracketing"), N_("Single frame bracketing"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonySingleFrameBracketingA100)},
    {0x0022, "WhiteBalanceBracketing", N_("White Balance Bracketing"), N_("White balance bracketing"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonyWhiteBalanceBracketingA100)},
    {0x0023, "WhiteBalanceSetting", N_("White Balance Setting"), N_("White balance setting"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonyWhiteBalanceSettingA100)},
    {0x0024, "PresetWhiteBalance", N_("Preset White Balance"), N_("Preset white balance"), sony1MltCsA100Id, makerTags, unsignedShort,
     1, EXV_PRINT_TAG(sonyPresetWhiteBalanceA100)},
    {0x0025, "ColorTemperatureSetting", N_("Color Temperature Setting"), N_("Color temperature setting"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonyColorTemperatureSettingA100)},
    {0x0026, "CustomWBSetting", N_("Custom WB Setting"), N_("Custom WB setting"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyCustomWBSettingA100)},
    {0x0027, "DynamicRangeOptimizerSettings", N_("Dynamic Range Optimizer Settings"), N_("Dynamic Range Optimizer Settings"),
     sony1MltCsA100Id, makerTags, unsignedShort, 1, printMinoltaSonyDynamicRangeOptimizerMode},
    {0x0032, "FreeMemoryCardImages", N_("Free Memory Card Images"), N_("Free memory card images"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, printValue},
    {0x0034, "CustomWBRedLevel", N_("Custom WB Red Level"), N_("Custom WB red level"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printValue},
    {0x0035, "CustomWBGreenLevel", N_("Custom WB Green Level"), N_("Custom WB green level"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, printValue},
    {0x0036, "CustomWBBlueLevel", N_("Custom WB Blue Level"), N_("CustomWB blue level"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printValue},
    {0x0037, "CustomWBError", N_("Custom WB Error"), N_("Custom WB Error"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyCustomWBErrorA100)},
    {0x0038, "WhiteBalanceFineTune", N_("White Balance Fine Tune"), N_("White balance fine tune"), sony1MltCsA100Id, makerTags,
     signedShort, 1, printValue},
    {0x0039, "ColorTemperature", N_("Color Temperature"), N_("Color temperature"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printValue},
    {0x003A, "ColorCompensationFilter", N_("Color Compensation Filter"), N_("Color compensation filter"), sony1MltCsA100Id, makerTags,
     signedShort, 1, printValue},
    {0x003B, "SonyImageSize", N_("Sony Image Size"), N_("Sony Image Size"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyImageSizeA100)},
    {0x003C, "Quality", N_("Quality"), N_("Quality"), sony1MltCsA100Id, makerTags, unsignedShort, 1, printMinoltaSonyQualityCs},
    {0x003D, "InstantPlaybackTime", N_("Instant Playback Time"), N_("Instant playback time"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, printValue},
    {0x003E, "InstantPlaybackSetup", N_("Instant Playback Setup"), N_("Instant playback setup"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonyInstantPlaybackSetupA100)},
    {0x003F, "NoiseReduction", N_("Noise Reduction"), N_("Noise reduction"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolValue},
    ...
    {0x0041, "RedEyeReduction", N_("Red Eye Reduction"), N_("Red eye reduction"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolValue},
    ...
    {0x0043, "AutoBracketOrder", N_("Auto Bracket Order"), N_("Auto bracket order"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyAutoBracketOrderA100)},
    {0x0044, "FocusHoldButton", N_("Focus Hold Button"), N_("Focus hold button"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyFocusHoldButtonA100)},
    {0x0045, "AELButton", N_("AEL Button"), N_("AEL button"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyAELButtonA100)},
    {0x0046, "ControlDialSet", N_("Control Dial Set"), N_("Control dial set"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyControlDialSetA100)},
    {0x0047, "ExposureCompensationMode", N_("Exposure Compensation Mode"), N_("Exposure compensation mode"), sony1MltCsA100Id,
     makerTags, unsignedShort, 1, EXV_PRINT_TAG(sonyExposureCompensationModeA100)},
    {0x0048, "AFAssist", N_("AF Assist"), N_("AF assist"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolInverseValue},
    {0x0049, "CardShutterLock", N_("Card Shutter Lock"), N_("Card shutter lock"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolInverseValue},
    {0x004A, "LensShutterLock", N_("Lens Shutter Lock"), N_("Lens shutter lock"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolInverseValue},
    {0x004B, "AFAreaIllumination", N_("AF Area Illumination"), N_("AF area illumination"), sony1MltCsA100Id, makerTags, unsignedShort,
     1, EXV_PRINT_TAG(sonyAFAreaIlluminationA100)},
    {0x004C, "MonitorDisplayOff", N_("Monitor Display Off"), N_("Monitor display off"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyMonitorDisplayOffA100)},
    ...
    {0x0050, "ExposureIndicator", N_("Exposure Indicator"), N_("Exposure indicator"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyExposureIndicatorA100)},
    {0x0051, "AELExposureIndicator", N_("AEL Exposure Indicator"),
     N_("AEL exposure indicator (also indicates exposure for next shot when bracketing)"), sony1MltCsA100Id, makerTags, unsignedShort,
     1, EXV_PRINT_TAG(sonyExposureIndicatorA100)},
    {0x0052, "ExposureBracketingIndicatorLast", N_("Exposure Bracketing Indicator Last"),
     N_("Exposure bracketing indicator last (indicator for last shot when bracketing)"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyExposureIndicatorA100)},
    {0x0053, "MeteringOffScaleIndicator", N_("Metering Off Scale Indicator"),
     N_("Metering off scale indicator (two flashing triangles when under or over metering scale)"), sony1MltCsA100Id, makerTags,
     unsignedShort, 1, EXV_PRINT_TAG(sonyMeteringOffScaleIndicatorA100)},
    {0x0054, "FlashExposureIndicator", N_("Flash Exposure Indicator"), N_("Flash exposure indicator"), sony1MltCsA100Id, makerTags,
    ...
    {0x0055, "FlashExposureIndicatorNext", N_("Flash Exposure Indicator Next"),
     N_("Flash exposure indicator next (indicator for next shot when bracketing)"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyExposureIndicatorA100)},
    {0x0056, "FlashExposureIndicatorLast", N_("Flash Exposure Indicator Last"),
     N_("Flash exposure indicator last (indicator for last shot when bracketing)"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyExposureIndicatorA100)},
    {0x0057, "ImageStabilization", N_("Image Stabilization"), N_("Image stabilization"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     printMinoltaSonyBoolValue},
    {0x0058, "FocusModeSwitch", N_("Focus Mode Switch"), N_("Focus mode switch"), sony1MltCsA100Id, makerTags, unsignedShort, 1,
     EXV_PRINT_TAG(sonyFocusModeSwitchA100)},
    ...
    {0x004B, "AELock", N_("AE Lock"), N_("AE lock"), sony1MltCsA100Id, makerTags, unsignedShort, 1, printMinoltaSonyBoolValue},
    {0x005E, "ColorTemperature", N_("Color Temperature"), N_("Color temperature"), sony1MltCsA100Id, makerTags, unsignedLong, 1,
     printValue},
    {0x005F, "ColorCompensationFilter", N_("Color Compensation Filter"),
     N_("Color compensation filter: negative is green, positive is magenta"), sony1MltCsA100Id, makerTags, unsignedLong, 1,
     printValue},
    ...
    // End of list marker
    {0xffff, "(UnknownSonyCsA100Tag)", "(UnknownSonyCsA100Tag)", N_("Unknown Sony Camera Settings A100 tag"), sony1MltCsA100Id,
     makerTags, unsignedShort, 1, printValue},
};
...
*/
extern TagDetails const minoltaSonyLensID[] = {{0, "Minolta AF 28-85mm F3.5-4.5 New"},
...
static std::ostream& resolveLens0xffff(std::ostream& os, const Value& value, const ExifData* metadata) {
  try {
    ...
    if (model == "ILCE-6000" && std::find(std::begin(maxApertures), std::end(maxApertures), maxAperture) != std::end(maxApertures))
      try {
        long focalLength = getKeyLong("Exif.Photo.FocalLength", metadata);
        if (focalLength > 0) {
          long focalL35mm = getKeyLong("Exif.Photo.FocalLengthIn35mmFilm", metadata);
...
constexpr TagDetails minoltaSonyColorMode[] = {{0, N_("Standard")},      {1, N_("Vivid Color")},  {2, N_("Portrait")},
...
constexpr TagDetails minoltaSonyLocalAFAreaPoint[] = {
    {1, N_("Center")},      {2, N_("Top")},  {3, N_("Top-Right")}, {4, N_("Right")},      {5, N_("Bottom-Right")}, {6, N_("Bottom")},
    {7, N_("Bottom-Left")}, {8, N_("Left")}, {9, N_("Top-Left")},  {10, N_("Far-Right")}, {11, N_("Far-Left")}};
...
constexpr TagDetails minoltaSonyQualityCs[] = {{0, N_("RAW")},       {2, N_("CRAW")},       {16, N_("Extra Fine")}, {32, N_("Fine")},
...
constexpr TagDetails minoltaSonyRotation[] = {{0, N_("Horizontal (normal)")}, {1, N_("Rotate 90 CW")}, {2, N_("Rotate 270 CW")}};
...
constexpr TagDetails minoltaSonySceneMode[] = {{0, N_("Standard")},    {1, N_("Portrait")},       {2, N_("Text")},
                                               {3, N_("Night Scene")}, {4, N_("Sunset")},         {5, N_("Sports")},
                                               {6, N_("Landscape")},   {7, N_("Night Portrait")}, {8, N_("Macro")},
                                               {9, N_("Super Macro")}, {16, N_("Auto")},          {17, N_("Night View/Portrait")}};
...
constexpr TagDetails minoltaSonyImageQuality[] = {{0, N_("Raw")},        {1, N_("Super Fine")},     {2, N_("Fine")},
                                                  {3, N_("Standard")},   {4, N_("Economy")},        {5, N_("Extra Fine")},
...
constexpr TagDetails minoltaSonyWhiteBalanceStd[] = {{0x00, N_("Auto")},     {0x01, N_("Color Temperature/Color Filter")},

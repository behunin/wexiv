...
LoaderExifJpeg::LoaderExifJpeg(PreviewId id, const Image& image, int parIdx) : Loader(id, image), offset_(0) {
  const auto exifData = image_.exifData();
  if (exifData.hasOwnProperty(param_[parIdx].offsetKey_) &&
      exifData.module_property(param_[parIdx].offsetKey_).as<size_t>() > 0) {
    offset_ = exifData.module_property(param_[parIdx].offsetKey_).as<size_t>();
  }

  size_ = 0;
  const auto sizeK = exifData.hasOwnProperty(param_[parIdx].sizeKey_);
  if (exifData.hasOwnProperty(param_[parIdx].sizeKey_) &&
      exifData.module_property(param_[parIdx].sizeKey_).as<size_t>() > 0) {
    size_ = exifData.module_property(param_[parIdx].sizeKey_).as<size_t>();
  }
  ...
  if (param_[parIdx].baseOffsetKey_) {
    if (exifData.hasOwnProperty(param_[parIdx].baseOffsetKey_) &&
        exifData.module_property(param_[parIdx].baseOffsetKey_).as<size_t>() > 0) {
      offset_ += exifData.module_property(param_[parIdx].baseOffsetKey_).as<size_t>();
    }
  }
...
LoaderExifDataJpeg::LoaderExifDataJpeg(PreviewId id, const Image& image, int parIdx) :
  ...
  const auto pos = image_.exifData();
  if (pos.hasOwnProperty(dataKey_.key().c_str())) {
    EXV_WARNING << "Exif Preview Data Not Implemented!";
  }
...
LoaderTiff::LoaderTiff(PreviewId id, const Image& image, int parIdx) :
  ...
  if (param_[parIdx].checkTag_) {
    if (!exifData.hasOwnProperty(param_[parIdx].checkTag_))
      return;
    if (param_[parIdx].checkValue_ &&
        exifData.module_property(param_[parIdx].checkTag_).as<std::string>() != param_[parIdx].checkValue_)
      return;
  }

  const auto sOff = std::string("Exif.") + group_ + ".StripOffsets";
  if (exifData.hasOwnProperty(sOff.c_str())) {
    offsetTag_ = "StripOffsets";
    sizeTag_ = "StripByteCounts";
    offsetCount = exifData.module_property(sOff.c_str()).as<size_t>();
  } else {
    const auto tOff = std::string("Exif.") + group_ + ".TileOffsets";
    if (!exifData.hasOwnProperty(tOff.c_str()))
      return;
    offsetTag_ = "TileOffsets";
    sizeTag_ = "TileByteCounts";
    offsetCount = exifData.module_property(tOff.c_str()).as<size_t>();
  }

  const auto sizeT = std::string("Exif.") + group_ + '.' + sizeTag_;
  if (!exifData.hasOwnProperty(sizeT.c_str()))
    return;
  if (offsetCount != exifData.module_property(sizeT.c_str()).as<size_t>())
    return;
  for (size_t i = 0; i < offsetCount; i++) {
    size_ += exifData.module_property(sizeT.c_str()).as<size_t>();  // FIXME!
  }

  if (size_ == 0)
    return;

  const auto iWidth = std::string("Exif.") + group_ + ".ImageWidth";
  if (exifData.hasOwnProperty(iWidth.c_str()) && exifData.module_property(iWidth.c_str()).as<size_t>() > 0) {
    width_ = exifData.module_property(iWidth.c_str()).as<size_t>();
  }

  const auto iLength = std::string("Exif.") + group_ + ".ImageLength";
  if (exifData.hasOwnProperty(iLength.c_str()) && exifData.module_property(iLength.c_str()).as<size_t>() > 0) {
    height_ = exifData.module_property(iLength.c_str()).as<size_t>();
  }
...
DataBuf LoaderTiff::getData() const {
  const auto image = image_.exifData();
  const auto oTag = "Exif.Image." + offsetTag_;
  const auto tmp = image.module_property(oTag.c_str());
  ExifData preview;

  Value& dataValue = tmp.as<Value&>();
...
LoaderXmpJpeg::LoaderXmpJpeg(PreviewId id, const Image& image, int parIdx) : Loader(id, image) {
  ...
  const auto xmpData = image_.xmpData();

  std::string prefix = "xmpGImg";
  if (xmpData.hasOwnProperty("Xmp.xmp.Thumbnails[1]/xapGImg:image")) {
    prefix = "xapGImg";
  }
  const auto img = "Xmp.xmp.Thumbnails[1]/" + prefix + ":image";
  if (!xmpData.hasOwnProperty(img.c_str()))
    return;
  auto imageDatum = xmpData.module_property(img.c_str());
  const auto format = "Xmp.xmp.Thumbnails[1]/" + prefix + ":format";
  if (!xmpData.hasOwnProperty(format.c_str()))
    return;
  auto formatDatum = xmpData.module_property(format.c_str());
  const auto width = "Xmp.xmp.Thumbnails[1]/" + prefix + ":width";
  if (xmpData.hasOwnProperty(width.c_str()))
    return;
  auto widthDatum = xmpData.module_property(width.c_str());
  const auto height = "Xmp.xmp.Thumbnails[1]/" + prefix + ":height";
  if (xmpData.hasOwnProperty(height.c_str()))
    return;
  auto heightDatum = xmpData.module_property(height.c_str());

  if (formatDatum.as<std::string>() != "JPEG")
    return;

  width_ = widthDatum.as<uint32_t>();
  height_ = heightDatum.as<uint32_t>();
  preview_ = decodeBase64(imageDatum.as<std::string>());
  size_ = preview_.size();
  valid_ = true;
}

...
float fnumber(float apertureValue) {
  float result = exp(log(2.0F) * apertureValue / 2.F);
...
URational exposureTime(float shutterSpeedValue) {
  URational ur(1, 1);
  double tmp = exp(log(2.0) * static_cast<double>(shutterSpeedValue));
  if (tmp > 1) {
...
uint16_t tagNumber(const std::string& tagName, IfdId ifdId) {
  ...
  if (ti != nullptr && ti->tag_ != 0xffff)
    return ti->tag_;
  if (!isHex(tagName, 4, "0x"))
    throw Error(kerInvalidTag, tagName, ifdId);
...
std::ostream& print0x0007(std::ostream& os, const Value& value, const ExifData*) {
  ...
  if (value.count() == 3) {
    ...
    std::ostringstream oss;
    oss.copyfmt(os);
    const float sec = 3600 * value.toFloat(0) + 60 * value.toFloat(1) + value.toFloat(2);
    int p = 0;
